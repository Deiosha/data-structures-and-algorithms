...
const toLastNames = people => {
  // Solution code here...
  let peopleArr = people.map(name => (name.firstName + ' ' + name.lastName));
  return peopleArr;
};
...
const addValues = (arr) => {
  // Solution code here...
  const sum = arr.reduce((value, num) => {
    return value + num;
  }, 0);
  return sum;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return arr.reduce(elements => elements + 1, 0);
};
...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((characters, value)=> {
...
const reversedString = (str) => {
  ...
  return newStr.reduce((accumlator, value) => {
    accumlator = value + accumlator;
    return accumlator;
  });
};
